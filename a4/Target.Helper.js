var TargetHelper = {
    ...
    getMapEnergies: function () {
        ...
        for (let roomName in Memory.map) {
            if (Memory.map.hasOwnProperty(roomName)) {
                ...
                if (room != null) {
                    var reservoirs = room.find(FIND_STRUCTURES, {
                        filter: (structure) => {
                    ...
                    for (let reservoir in reservoirs) {
                        var instance = reservoirs[reservoir].id;
                        console.log(instance);
                        if (instance != null) {
                            energies.push(instance);
                        }
                    }
                }
            }
    ...
    getEnergyReservoir: function () {
        ...
        for (let room in Memory.map) {
            if (Memory.map.hasOwnProperty(room)) {
                ...
                if (currentRoom) {
                    var reserve =
                        currentRoom.find(FIND_STRUCTURES, {
                            filter: (structure) => {
    ...
    getGatherableEnergies: function () {
        var sources = TargetHelper.getMapEnergies();
        var reservoirs = [];
        for (let room in Memory.map) {
            if (Memory.map.hasOwnProperty(room)) {
                let currentRoom = Game.rooms[room];
