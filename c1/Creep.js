...
var Creep = {
    ...
    ACTION_LISTS: ['harvest', 'harvest', 'harvest', 'build', 'build', 'upgrade', 'scout', 'moveRoom', 'moveRoom'],
    ...
    run: function (creep) {
        ...
        if (!MemoryWrapper.isCreepLocked(creep)) {
            ...
            if (creep.room.controller && creep.room.controller.my && creep.room.controller.ticksToDowngrade < 1500) {
                var currentAction = 'upgrade';
            }
            ...
            case 'claim':
                ...
                if (target.id && Game.getObjectById(target.id).structureType) {
                    if (target.structureType === STRUCTURE_CONTROLLER) {
                        var controller = Game.getObjectById(target.id);
                        var exec = undefined;
                        if (controller.reservation) {
                            console.log('attacking controller');
                            exec = creep.attackController(controller);
                        } else {
                            console.log('claiming');
                            exec = creep.claimController(Game.getObjectById(target.id));
                        }
                        if (exec === OK) {
                            var map = {
