var Weight = {
    ...
    update: function (name, update_delta, update_label) {
        ...
        var diff = ((probs[x] * update_delta) - probs[x]) / (n - 1);
        probs[x] = probs[x] * update_delta;
        // if(probs[x] > 1) probs[x] = 1;
        for (var i = 0; i < n; i++) {
            if (i !== x) {
                probs[i] += diff;
            }
            if (probs[i] <= 0) probs[i] = 0.0000001;
        }
        var adjustment = Weight.adjustByFilter(name);
        var p = adjustment.probs;
        Memory.weight[name].probs = p;
        // console.log(p);
    },
